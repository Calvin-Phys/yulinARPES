...
handles.mObject = figure( 'Position', [100 50 300 350], ...
    'Name', 'LoadDataARPES',...
    'NumberTitle', 'off', ...
    'menubar', 'none',...
    'Units','Pixel',...
...
for i=1:n
    ...
    try
        if load_CASTEP(fullfile(cd,name))
        ...
        %I comment these two below because it takes forever to load spin
        %ape file --- please somebody check it
        elseif load_scienta_general_txt_BZ(fullfile(cd,name))
