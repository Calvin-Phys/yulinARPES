...
if ~strcmpi(instrumentName,'i05')
% Identify Diamond Data i05
...
% cuts
if max(size(size(data.value)))== 4
    testflag=0;
    dimension1='sax';
    dimension2='saz';
    if testflag==1
        %% Written by Zk Liu.
        value = zeros(size(data.value,4),size(data.value,3));
        value = squeeze(sum(sum(data.value(:,1:734,:,:),1),2))';%number 734 is to truncate strange edge from quick cuts
        clear data;
        data.value=value;
        try
            temp=h5read(varargin{1},'/entry1/analyser/sax');
            data.x=temp(1,:)';
        catch
        end
        try
            temp=h5read(varargin{1},'/entry1/analyser/saz');
            data.y=temp(:,1);
        catch
        end
       %%
        ...
        %% -------4d data  x-y scan ------------------
        try
            data.value=permute(data.value,[4,3,2,1]);
        ...
        try
            data.k = h5read(varargin{1},'/entry1/analyser/angles');
        catch
        end
        try
