function v_return=load_sampled_soleil_nxs(varargin)
...
%here 'start,count,stride' 1,2,3,4 indicate z,k,y,x,
%output data.value(x,y,k,z)
...
if n>0     % if '/Scienta_0' exists, then it is map data.
    ...
    %select data from whole map data
    count=count-1;
    data.x=data.x(start(4):stride(4):(start(4)+count(4)*stride(4)));
    data.y=data.y(start(3):stride(3):(start(3)+count(3)*stride(3)));
    data.z=data.z(start(1):stride(1):(start(1)+count(1)*stride(1)));
    data.value=data.value(start(4):stride(4):start(4)+count(4)*stride(4),...
        start(3):stride(3):start(3)+count(3)*stride(3),...
        start(1):stride(1):start(1)+count(1)*stride(1));
end
...
if ~isempty(idx)
    ...
    if ~isempty(idx)
        ...
        M4=h5read(fullfilename,[rootPath '/scan_data/data_04'],start,count,stride);
    else
        data.info.DataType='PIx-ZPs';
        M4=h5read(fullfilename,[rootPath '/scan_data/data_02'],start,count,stride);
    end
    data.x=h5read(fullfilename,[rootPath '/scan_data/trajectory_2_1'],...
        start(1,4),count(1,4),stride(1,4));
    data.y=h5read(fullfilename,[rootPath '/scan_data/trajectory_1_1'],...
        start(1,3),count(1,3),stride(1,3));
    
    try
        data.k= h5read(fullfilename,[rootPath,...
            '/ANTARES/ScientaAtt1/sliceScale/data'],...
            start(1,2),count(1,2),stride(1,2));
    catch
        data.k=start(2):stride(2):(start(2)+stride(2)*(count(2)-1));
    end
    try
        data.z= h5read(fullfilename,[rootPath,...
            '/ANTARES/ScientaAtt1/channelScale/data'],...
            start(1,1),count(1,1),stride(1,1));
    catch
        data.z=start(1):stride(1):(start(1)+stride(1)*(count(1)-1));
    end
    try % Deal with memory limitation
        data.value=permute(M4,[4 3 2 1]);  %data.value(x,y,k,z)
    catch ME
