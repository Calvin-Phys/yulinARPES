function tf = load_PEARLv02(varargin)
...
try
    try
        low=h5read([filename '.h5'],'/scan 1/attrs/ScientaLowEnergy');
    catch
        low=h5read([filename '.h5'],'/scan1/attrs/ScientaLowEnergy');
    end
catch
    tf=false;
    return
    ...
    [Angle,Energy,Scan,Data,ep,Note]=ReadPEARLv2([filename '.h5']);
...
%     datav_new.info.Note=Note;
    ...
    assignin('base',strcat('PL',filename),datav_new);
