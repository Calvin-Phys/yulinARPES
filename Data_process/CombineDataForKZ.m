...
% Last Modified by GUIDE v2.5 01-Oct-2016 12:58:53
...
for i = 1:size(index_selected,2)
    ...
    % combining
    datafin.x = cat(2,datafin.x,datain1.x);
    datafin.y = datain1.y;
    datafin.z = datain1.z;
    datafin.value=cat(1,datafin.value,datain1.value);
    datafin.ztot= cat(2,datafin.ztot,datain1.ztot);
end
...
assignin('base','Combined3DPhotonDependence',datafin);
display('combine 3D PhotonData done')
load chirp;
sound(y, Fs);
...
for i = 1:size(index_selected,2)
    ...
    if ~isstruct(datain1)
        errordlg(['data',num2str(i),'is not struct'])
        return;
    elseif (~(isfield(datain1,'x') && isfield(datain1,'y') && isfield(datain1,'value')))
        errordlg(['data',num2str(i),'is wrong. Data needs at least x,y,value field']);
        return;
    elseif (~(isfield(datain1.info,'PhotonEnergy')))
        errordlg(['please check where is the PhotonEnergy data of data ',var_name1,' and put in data.info.PhotonEnergy']);
        return;
    end
    % combining
    datafin.x(i) = datain1.info.PhotonEnergy;
    datafin.y = datain1.x;
    datafin.z = datain1.y;
    datafin.value(i,:,:)=datain1.value;
    datafin.ztot(:,i)= datain1.y;
end
...
assignin('base','CombinedSlicesPhoton',datafin);
display('combine slices done')
load chirp;
sound(y, Fs);
