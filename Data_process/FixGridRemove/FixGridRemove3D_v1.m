function [data_nu, data_fft, data_fft_full_inan_remove, debug] = FixGridRemove3D_v1(data, direction, opts)
%FIXGRIDREMOVE3D_V1 Remove 2D grid-like background from a 3D stack slice-by-slice.
%
% Usage:
%   [data_nu, data_fft, data_fft_full_inan_remove, debug] = ...
%       FixGridRemove3D_v1(data, 'z', opts);
%
% Inputs
...
%   opts.debugSlices    : slice indices to visualize, default first/mid/last when debug=true
%
% Outputs
...
%   debug                    : diagnostics, including per-slice peak positions

    if nargin < 2 || isempty(direction)
        error('FixGridRemove3D_v1:MissingDirection', 'You must provide direction = ''x'', ''y'', or ''z''.');
    end
    ...
    debugSlices = getOpt(opts, 'debugSlices', []);
    ...
    if isempty(refRefinedPeaks) || size(refRefinedPeaks,1) ~= 2 || size(refRefinedPeaks,2) ~= 2
        warning('FixGridRemove3D_v1:NoReferencePeaks', 'Could not determine two reference peaks. Returning input unchanged.');
        data_nu.value = V;
    ...
    if rcond(refB) < 1e-10 || abs(det(refB)) < 1e-8
        warning('FixGridRemove3D_v1:DegenerateReferenceBasis', 'Reference peak pair is degenerate. Returning input unchanged.');
        data_nu.value = V;
        data_fft_full_inan_remove.x = plane.uAxis;
        data_fft_full_inan_remove.y = plane.vAxis;
        data_fft_full_inan_remove.value = Aref;
    ...
    % A few representative slices for debug figures
    if debugMode && isempty(debugSlices)
        debugSlices = defaultDebugSlices(plane.nStack);
    end
    debug.debugSlices = debugSlices(:).';

    % For all slices, refine around the same reference peaks
    % This is the stable "reference-seeded" mode requested.
    Vout = V;
    ...
    for is = 1:plane.nStack
        ...
        % If refinement collapses, fall back to the reference basis
        ...
        % Build and apply the smooth notch mask for this slice
        [mask, F0, Ffilt] = buildNotchFilteredFFT(sliceFilled, B, maxOrder, centerFrac, sigmaFactor, notchDepth, minMask);
        ...
        if debugMode && ismember(is, debugSlices)
            showSliceDebugFigure(figBase + 100 + is, sliceFilled, A, mask, log1p(abs(Ffilt)), ...
                sliceGuess, refinedPeaks, B, plane, is, sliceClean, status);
        end
    end

    data_nu.value = Vout;

    % Reference-plane FFT after applying the reference mask
    [refMask, refF0, refFfilt] = buildNotchFilteredFFT(refPlaneFilled, refB, maxOrder, centerFrac, sigmaFactor, notchDepth, minMask);
    data_fft_full_inan_remove.x = plane.uAxis;
    data_fft_full_inan_remove.y = plane.vAxis;
    ...
    if debugMode
        showReferenceDebugFigure(figBase, refPlaneFilled, Aref, refMask, log1p(abs(refFfilt)), ...
            refInitialPeaks, refRefinedPeaks, refB, plane, refMode);
    end
end
    ...
    switch refMode
        ...
        case 'slice'
            if isempty(refSliceIdx) || ~isscalar(refSliceIdx) || ~isfinite(refSliceIdx)
                error('FixGridRemove3D_v1:MissingRefSliceIndex', ...
        ...
        case 'matrix'
            if isempty(refMatrix) || ~isnumeric(refMatrix) || ndims(refMatrix) ~= 2
                error('FixGridRemove3D_v1:MissingRefMatrix', ...
            ...
            if ~isequal(size(refPlane), [numel(plane.uAxis), numel(plane.vAxis)])
                if isequal(size(refPlane), [numel(plane.vAxis), numel(plane.uAxis)])
                    ...
                    error('FixGridRemove3D_v1:RefMatrixSizeMismatch', ...
                        'refMatrix size must match the selected plane size, or its transpose.');
                end
            end

        otherwise
            error('FixGridRemove3D_v1:BadRefMode', 'opts.refMode must be "average", "slice", or "matrix".');
    end
end
...
function p = refineFFTpeak(A, xClick, yClick, searchRad, fitRad)
    ...
    % centered FFT coords -> matrix indices
    r0 = yClick + cx;
    c0 = xClick + cy;
    ...
    % Quadratic fit for subpixel localization
...
function [mask, F0, Ffilt] = buildNotchFilteredFFT(sliceFilled, B, maxOrder, centerFrac, sigmaFactor, notchDepth, minMask)
...
function showReferenceDebugFigure(figNum, refPlaneFilled, Aref, mask, Afinal, initialPeaks, refinedPeaks, B, plane, refMode)
...
function showSliceDebugFigure(figNum, sliceFilled, A, mask, Afinal, initialPeaks, refinedPeaks, B, plane, sliceIndex, sliceClean, status)
    [nx, ny] = size(sliceFilled);
    cx = (nx + 1) / 2;
    cy = (ny + 1) / 2;
    ...
    nexttile;
    imagesc(plane.vAxis, plane.uAxis, sliceFilled);
    axis image;
    set(gca, 'YDir', 'normal');
    title(sprintf('Slice %d input (%s)', sliceIndex, status));
    xlabel(plane.vLabel);
    ylabel(plane.uLabel);
    colorbar;
...
function s = defaultDebugSlices(nStack)
    if nStack <= 1
        s = 1;
    elseif nStack == 2
        s = [1 2];
    else
        s = unique([1, round(nStack/2), nStack]);
    end
end
