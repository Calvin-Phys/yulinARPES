function data3D_nu = FixGridRemove_3d(data3D)
% data3D: input 3D map
% data3D_nu: output grid-removed data

p1 = 0.95; % intensity threshold parameter: higher value will result in less fft grid pattern found. 0.8<p1<1
p2 = 0.9; % intensity threshold parameter: higher value will result in less area to be set nan. 0.8<p1<1
p3 = 0.6; % region of interest parameter, 0.3<p3<0.7

data3D_nu = data3D;

Num = length(data3D.x);
Nmid = round(Num/2);
data.value = squeeze(data3D.value(Nmid,:,:));
data.x = data3D.y;
data.y = data3D.z;
...
shading interp;
display('first zoom in, then press enter to continue')
while true
...
display('select two first order point');
[xi,yi] = getpts;
...
mask = isnan(data_fft_full.value);

for ii = 1:Num
    data.value = squeeze(data3D.value(ii,:,:));
    data.x = data3D.y;
    data.y = data3D.z;
...
%---calculate log fft---%
    ...
    value_fft = fftshift(fft2(value));
    data_fft_full.value = value_fft;
    data_fft_full.value(mask) = nan;
    data_fft_full_inan = InterpNaNs(data_fft_full);
    data3D_nu.value(ii,:,:) = abs(ifft2(fftshift(data_fft_full_inan.value)));
end
