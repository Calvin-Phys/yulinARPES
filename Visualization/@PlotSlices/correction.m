...
if (A(1,1)>0) %positive quadratic term coefficient
    ...
    for i=1:sizeOfxData
        dif=fix((polyval(A,xd(i))-fitmin1)*sizeOfyData/rangeOfyData);%difference
        nsliceData(i,newyDataDimension-dif-sizeOfyData+(1:sizeOfyData))=sliceData(i,1:sizeOfyData);
    end
    if (d==1)
        columnsize=max(size(obj.Data.x));
    ...
    if (d==2)
        columnsize=max(size(obj.Data.y));
...
if (A(1,1)<0)%negative quadratic term coefficient
    ...
    nsliceData=NaN.*ones(sizeOfxData,newyDataDimension);
    for i=1:sizeOfxData
        dif=fix((fitmax2-polyval(A,xd(i)))*sizeOfyData/rangeOfyData);%difference
...
if (flagL+flagU~=0)
    ...
    for i=1:newyDataDimension
        nyData(i)=min(yData)+(i-1)*rangeOfyData/sizeOfyData;%new yData
        %sandy modified
        if isfield(obj.Data,'ztot')
