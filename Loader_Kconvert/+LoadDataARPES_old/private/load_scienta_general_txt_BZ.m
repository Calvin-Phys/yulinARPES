...
while ~strncmp(info_line,'[Data',5)
    ...
    elseif strncmp(info_line, 'Polar=', 6)
        data.info.theta=sscanf(info_line(7:end),'%f');
    elseif strncmp(info_line, 'phi=', 4)
        data.info.phi=sscanf(info_line(5:end),'%f');
    elseif strncmp(info_line, 'Tilt=', 5)
        data.info.phi=sscanf(info_line(6:end),'%f');
    elseif strncmp(info_line, 'azi=', 4)
        data.info.azi=sscanf(info_line(5:end),'%f');
    elseif strncmp(info_line,'Azimuth=',8)
        data.info.azi=sscanf(info_line(9:end),'%f');
...
if isempty(scale3)  % 1D and 2D data can be input the same way
    ...
    %
    % Added by Han Peng 20151016 for BESSY 1^2 Beamline
    datavalue=zeros(length(scale3),length(scale1),length(scale2));
    for k=1:length(scale3)
        for i=1:length(scale1)
            line_data=sscanf(fgetl(fid),'%f');
            if ~isempty(line_data)
                datavalue(k,i,:)=line_data(2:length(line_data));
            end
        end
    end
    data.value=permute(datavalue,[1,3,2]);
    % End of modification
end
fclose(fid);
%--------------end of getting data----------------------------

%write filename into info field
data.info.file_name=file_name;
