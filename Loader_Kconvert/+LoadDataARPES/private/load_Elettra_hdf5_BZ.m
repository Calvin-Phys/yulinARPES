function tf=load_Elettra_hdf5_BZ(varargin)
...
if ~strcmpi(ext,'.hdf5')
%     errordlg(['The file "' varargin{1} '" is not a .hdf5 file!'],'Wrong file format');
    tf=false;
    return
end
...
if max(size(size(data.value)))==3
    ...
    % set contrast
    n_data = sort(data.value(:));
    upbound = n_data(round(0.995*length(n_data)));
    data.value(data.value>upbound) = upbound;
 %--------------------2D data--------------------    
